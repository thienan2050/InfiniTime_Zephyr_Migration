...
namespace Pinetime {
  namespace Applications {
    ...
    template <Apps t>
    constexpr AppDescription CreateAppDescription() {
      return {AppTraits<t>::app, AppTraits<t>::icon, &AppTraits<t>::Create, &AppTraits<t>::IsAvailable};
    }

    template <WatchFace t>
    constexpr WatchFaceDescription CreateWatchFaceDescription() {
    ...
    constexpr std::array<AppDescription, sizeof...(ts)> CreateAppDescriptions(T<ts...>) {
      return {CreateAppDescription<ts>()...};
    }

    template <template <WatchFace...> typename T, WatchFace... ts>
    constexpr std::array<WatchFaceDescription, sizeof...(ts)> CreateWatchFaceDescriptions(T<ts...>) {
