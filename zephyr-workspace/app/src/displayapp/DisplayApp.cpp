...
void DisplayApp::Start(System::BootErrors error) {
   //Replace by Zephyr OS: msgQueue = xQueueCreate(queueSize, itemSize);

   //Replace by Zephyr OS: bootError = error;

   //Replace by Zephyr OS: if (pdPASS != xTaskCreate(DisplayApp::Process, "displayapp", 800, this, 0, &taskHandle)) {
     //Replace by Zephyr OS: APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
   //Replace by Zephyr OS: }
}

void DisplayApp::Process(void* instance) {
  auto* app = static_cast<DisplayApp*>(instance);
   //Replace by Zephyr OS: NRF_LOG_INFO("displayapp task started!");
  app->Init();

  if (app->bootError == System::BootErrors::TouchController) {
    app->LoadNewScreen(Apps::Error, DisplayApp::FullRefreshDirections::None);
  } else {
    app->LoadNewScreen(Apps::None, DisplayApp::FullRefreshDirections::None);
  }
...
void DisplayApp::Refresh() {
    //if (state == States::Running) {
     //Replace by Zephyr OS: lv_task_handler();
  //}
...
void DisplayApp::LoadScreen(Apps app, DisplayApp::FullRefreshDirections direction) {
  lvgl.CancelTap();
   //Replace by Zephyr OS: lv_disp_trig_activity(nullptr);
  ...
  switch (app) {
    ...
    default: {
      ...
      // });
      auto d = std::find_if(userApps.begin(), userApps.end(),
                      [app](const AppDescription& appDescription) {
                        return appDescription.app == app;
                      });
