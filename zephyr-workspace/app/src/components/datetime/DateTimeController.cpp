...
namespace {
  ...
  inline int compileTimeAtoi(const char* str) {
    int result = 0;
    while (*str >= '0' && *str <= '9') {
...
DateTime::DateTime(Controllers::Settings& settingsController) : settingsController {settingsController} {
  //Replace by Zephyr OS: mutex = xSemaphoreCreateMutex();
  //Replace by Zephyr OS: ASSERT(mutex != nullptr);
  //Replace by Zephyr OS: xSemaphoreGive(mutex);
...
void DateTime::SetCurrentTime(std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds> t) {
  //Replace by Zephyr OS: xSemaphoreTake(mutex, portMAX_DELAY);
...
void DateTime::SetTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second) {
  ...
  //Replace by Zephyr OS: NRF_LOG_INFO("%d %d %d ", day, month, year);
  //Replace by Zephyr OS: NRF_LOG_INFO("%d %d %d ", hour, minute, second);
...
std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds> DateTime::CurrentDateTime() {
  //Replace by Zephyr OS: xSemaphoreTake(mutex, portMAX_DELAY);
  UpdateTime(nrf_rtc_counter_get(NRF_RTC1), false);
  //Replace by Zephyr OS: xSemaphoreGive(mutex);
...
void DateTime::UpdateTime(uint32_t systickCounter, bool forceUpdate) {
  ...
  if (systickCounter < previousSystickCounter) {
    systickDelta = static_cast<uint32_t>(0xFFFFFF) - previousSystickCounter;
  ...
  auto correctedDelta = systickDelta / CONFIG_SYS_CLOCK_TICKS_PER_SEC;
  ...
  auto rest = systickDelta % 1000;
  if (systickCounter >= rest) {
    previousSystickCounter = systickCounter - rest;
  } else {
    previousSystickCounter = static_cast<uint32_t>(0xFFFFFF) - (rest - systickCounter - 1);
  }
