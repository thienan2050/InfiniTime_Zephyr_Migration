...
void TwiMaster::Init() {
  //Replace by Zephyr OS: if (mutex == nullptr) {
  //Replace by Zephyr OS:   mutex = xSemaphoreCreateBinary();
...
TwiMaster::ErrorCodes TwiMaster::Write(uint8_t deviceAddress, uint8_t registerAddress, const uint8_t* data, size_t size) {
  //Replace by Zephyr OS: ASSERT(size <= maxDataSize);
  //Replace by Zephyr OS: xSemaphoreTake(mutex, portMAX_DELAY);
  Wakeup();
...
void TwiMaster::FixHwFreezed() {
  //Replace by Zephyr OS: NRF_LOG_INFO("I2C device frozen, reinitializing it!");
