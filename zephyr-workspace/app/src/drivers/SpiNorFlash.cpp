...
void SpiNorFlash::Init() {
  ...
  //Replace by Zephyr OS: NRF_LOG_INFO("[SpiNorFlash] Manufacturer : %d, Memory type : %d, memory density : %d",
               //Replace by Zephyr OS: device_id.manufacturer,
               //Replace by Zephyr OS: device_id.type,
               //Replace by Zephyr OS: device_id.density);
}
...
void SpiNorFlash::Sleep() {
  ...
  //Replace by Zephyr OS: NRF_LOG_INFO("[SpiNorFlash] Sleep")
}
...
void SpiNorFlash::Wakeup() {
  ...
  if (devId.type != device_id.type) {
    //Replace by Zephyr OS: NRF_LOG_INFO("[SpiNorFlash] ID on Wakeup: Failed");
  } else {
    //Replace by Zephyr OS: NRF_LOG_INFO("[SpiNorFlash] ID on Wakeup: %d", id);
  }
  //Replace by Zephyr OS: NRF_LOG_INFO("[SpiNorFlash] Wakeup")
}
...
void SpiNorFlash::SectorErase(uint32_t sectorAddress) {
  ...
  WriteEnable();
  while (!WriteEnabled())
   //Replace by Zephyr OS: vTaskDelay(1);
